...
class WOFComputerPlayer(WOFPlayer):
    ...
    def smartCoinFlip(self):
        rdm_num = random.randint(1, 10)
        if rdm_num > self.difficulty:
            return True
        else:
            return False
    ...
    def getMove(self, category, obscurePhrases, guessed):
        letters = self.guessedPossibleLetters(guessed)
        if 0 == len(letters):
            return 'pass'
        scf = self.smartCoinFlip()
        if scf is True:
            # [::-1] reverses SORTED FREQUENCIES because need to find highest indexed available letter
